...
class FCNode : public canopen::FiberDriver
{
public:
  using FiberDriver::FiberDriver;

  FCNode(ev_exec_t *exec, canopen::AsyncMaster &master, unsigned int id, std::string ControllerBrand, double encoderRes,
         double gearRatio, double VelocityFactor, unsigned int commandPeriod, OpMode operationMode, double maxAcc, double maxVel,
         double current_threshold, double vel_findlimit, std::shared_ptr<SharedState> sharedState,
...
private:
  ...
  void TaskTargetFC() noexcept;
  int FaulhaberCommand(std::string command, int value);
  ...
  void SetProfileParams(int maxACC, int maxDCC, int maxSpeed);
  void SetEncoder(const double offset);
  void FindFowrardLimits();
  int FaulhaberTraceConfig();
...
private:
  ...
  int m_profile_acc_SI;         // in SI unit
  int m_profile_vel_SI;         // in SI unit  (only for position profile (PP) mode)
